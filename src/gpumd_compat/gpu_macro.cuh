...
/*----------------------------------------------------------------------------
Origin file: src/utilities/gpu_macro.cuh
Purpose: CUDA/HIP abstraction macros used by the replicated GPUMD sources
(only the CUDA branch can ever be active in DMG-MD); verbatim.
...
// NOTE(dmg-md): the HIP branch below is inherited verbatim from the reference
// but can never activate in DMG-MD (USE_HIP is not defined anywhere in this
// build); only the CUDA mappings are reachable.
