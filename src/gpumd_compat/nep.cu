...
/*----------------------------------------------------------------------------
Origin file: src/force/nep.cu
Purpose: NEP potential-file loader, large-box kernel orchestration, and the
...
// Element symbol table indexed by atomic number-1; used to resolve the
// potential header symbols to Z for the ZBL terms (reference nep.cu:37).
...
// Repacks the descriptor parameters from the file layout (basis-major) into
// the type-pair layout consumed by annmb.c_type_pair inside the kernels
// (reference nep.cu:75).
...
// Filters the global Verlet list into typewise radial and angular ELL lists
// (one thread per center in [N1,N2)); cutoffs are type-pair averages
// (reference nep.cu:436).
...
// One thread per center: radial descriptors from the radial list, angular
// moments -> invariants q, q-scaler normalization, ANN evaluation giving
// the site energy and Fp (descriptor derivatives); writes Fp and sum_fxyz
// for the force kernels (reference nep.cu:488).
...
// Radial contribution to per-atom force/virial; reads Fp of both the
// center and each neighbor (reference nep.cu:661).
...
// Computes the directed angular partial forces f12(i,j) for every angular
// edge of centers in [N1,N2) using Fp and sum_fxyz of the center; consumed
// by the many-body gather (reference nep.cu:774).
...
// Optional ZBL repulsion on the angular list: per-atom force/virial/PE
// with either the universal or flexible parameters (reference nep.cu:863).
...
// Large-box orchestration: Verlet update -> typewise filter -> descriptor ->
// radial force -> angular partial -> many-body gather -> optional ZBL.  Also
// appends the neighbor occupancy to neighbor.out every 1000 calls; DMG-MD's
// RankIoIsolation redirects that file for non-root ranks (reference
// nep.cu:978).
...
// Small-box orchestration over explicit periodic images with atomic
// Newton-scatter force kernels (reference nep.cu:1141).
...
// Decides small-box vs large-box: any periodic direction thinner than
// 2.5*(rc+1) requires image expansion; builds the expanded box matrix and
// its inverse in ebox (reference nep.cu:1295).
...
// Dispatches between the small-box (explicit periodic images + atomic
// Newton scatter) and large-box (Verlet cell list + center gather) kernel
// families.  A periodic direction thinner than 2.5*(rc+1) forces the
// small-box path (see get_expanded_box above).  DMG-MD calls this from
// NepForce::compute in src/runtime.cu with N1=0 and N2=local_count.
// NOTE(dmg-md): the DFTD3 tail of the reference implementation
// (nep.cu:1385) is dropped because DMG-MD rejects the 'dftd3' run.in
// keyword as unsupported before construction.
void NEP::compute(
  Box& box,
  const GPU_Vector<int>& type,
  const GPU_Vector<double>& position_per_atom,
