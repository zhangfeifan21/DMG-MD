...
/*----------------------------------------------------------------------------
Origin file: src/force/nep_small_box.cuh
Purpose: Small-box NEP kernels: explicit-image neighbor lists and the atomic
Newton-scatter force kernels used when a periodic direction is
thinner than 2.5 radial cutoffs.
...
// Minimum image in the expanded box's fractional space; keeps the explicit
// image offset valid (reference nep_small_box.cuh:39).
...
// One thread per center: enumerates every atom x every expanded-box image,
// keeping pairs inside the typewise radial/angular cutoffs and storing the
// explicit r12 vectors (reference nep_small_box.cuh:56).
...
// Descriptor + ANN kernel for the small-box path; identical math to the
// large-box find_descriptor but reading the stored r12 instead of global
// positions (reference nep_small_box.cuh:134).
...
// Radial force with Newton's-third-law scatter: adds +/-f12 to both atoms
// and the virial to n2 via double atomicAdd (reference
// nep_small_box.cuh:398).
...
// Angular force with Newton scatter, computed per edge from Fp/sum_fxyz of
// the center (reference nep_small_box.cuh:496).
...
// ZBL force with Newton scatter on the angular list (reference
// nep_small_box.cuh:620).
